...
整数端口和整数变量默认值支持完整的 Go `int64` 范围。为兼容旧文件和 JavaScript：`Number` 可精确表示的整数继续保存为 JSON number，新输入或编辑的超安全范围整数在 `.obp/.obpf` 中保存为十进制字符串；读取旧文件中未加引号的超安全范围整数时，编辑器使用内部精确整数标记保留原始十进制文本及 JSON number 类型。已知 Integer 控件会把该标记转换为可编辑的十进制字符串，未知 legacy 字段和 `Any` 数据则在打开、撤销及保存后仍输出原数值 token，不会被转成字符串或经过 JavaScript `Number` 丢失精度。Go 校验器和编译器同时接受 number 与十进制字符串两种 Integer 表示，导出 `.vgf` 时恢复为旧格式的 JSON 数字。旧编辑器保存的节点 Integer 空字符串继续按 `0` 读取并在编辑时归一化；Integer 变量的空字符串仍是严重错误。小数和超出 int64 范围的值也属于严重默认值错误。`itemType` 元数据和嵌套数组暂不写入现有文件格式，待类型契约稳定后再以可选字段设计，缺省值必须继续表示 legacy 任意项。
