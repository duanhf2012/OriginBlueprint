...
桌面编辑器按“内嵌节点库 → 编辑器可执行文件同级 `nodes/` → 当前工作目录 `nodes/` → 已打开工作区 `<workspace>/nodes`”的顺序合并节点定义，后面的来源按相对路径覆盖前面的同名文件。打开或切换工作区时会自动刷新节点库，F5 的 `target.go` 校验使用相同来源；新增或修改 JSON 后，在编辑器“文件”菜单选择“刷新节点库”，无需重启编辑器。
...
- 在节点库双击或拖拽节点到画布。
- 在画布右键搜索节点。
- 普通单击节点会切换为单选；按住 `Ctrl` 单击可累计多选。拖动已框选节点中的任意一个会移动整组，仅单击而未拖动时会收束为当前节点。
- 从输出端口拖到类型兼容的输入端口创建连线。普通 Exec 输出再次连接时以新连线替换该端口的旧连线，并作为一次操作支持 Undo/Redo；载入已有 legacy `.vgf` 时不会自动删除历史 fan-out，需要保留原图并由用户显式修正或插入 Sequence。
- Exec 输入允许多个上游执行分支汇入；任一上游到达时都会执行该节点。数据输入仍只允许一个生产者。
...
- Exec 端口只连接 Exec 端口。
- 多个 Exec 输出可以汇入同一个 Exec 输入；一个普通 Exec 输出不能直接连接多个目标，需要按顺序执行多个目标时使用 Sequence。
- 数据端口应保持相同类型；编辑器只对少量明确转换提供自动转换。
- 数据流描述“取值来源”，执行流描述“执行顺序”。数据连线本身不会保证有副作用节点已经执行。
- 有副作用的数据生产节点必须接入执行流，再由后续节点读取其输出。
- 同一图中的入口节点按节点 ID 稳定排序，依次使用内置的 50 色高区分度色板；入口菱形、入口数据端口、引用该入口参数的目标端口/标签，以及显式显示的入口参数连线使用同一种颜色。超过 50 个入口时色板循环使用。
...
- 变量名称使用 UTF-8，可直接使用中文；名称不能为空，同一蓝图内不能重复。原生 Getter/Setter 通过稳定变量 ID 关联，编辑名称时画布上已有节点的标题和端口标签会实时更新，不会重建节点或改变连线与位置；Go 编译器和运行时同样支持中文变量名。
- 变量分组只用于编辑器内组织变量，不参与运行时语义；局部与全局变量各自维护分组，可使用相同的分组名称。两栏分别提供新增变量和新增分组操作；变量切换作用域时优先进入目标作用域中的同名分组，否则进入 Default。删除分组只影响该分组中的变量，并把它们移动到对应作用域的 Default。
- 变量可直接拖到同一作用域的其他分组，或拖到局部/全局标题区域进入对应 Default；该操作不改变变量 ID 和文档中的变量顺序。跨局部与全局拖动属于作用域变更，必须确认后才会同时更新 `scope` 和目标 `groupId`。局部转全局后同一实例并发执行共享当前值；全局转局部后新执行从默认值初始化。函数图拒绝拖入全局区域，`.vgf` 转为全局变量后必须另存为 `.obp`。
- 旧 `.obp` 中没有 `variableGroups[].scope` 的分组会按实际变量推断作用域：只含局部或全局变量时归入对应栏，同时含两种变量时拆分为两个同名分组，空分组归入局部栏。旧 `.vgf` 的变量 `group` 迁移为局部分组，并在导回 `.vgf` 时按当前变量与分组状态写回；变量中的未知 legacy 扩展字段继续保留。分组名称在各自作用域内按忽略大小写判重，避免编辑器归一化时静默合并内容。
...
- 打开原生 `.obp/.obpf` 时会先校验原始 JSON，再做编辑器归一化；原始内容存在明确的核心致命错误，或恢复时确认丢失节点/连线时，原路径进入保护状态，只允许另存副本。显式“强制保存”会先原子写入同目录 `.bak` 备份，但不能绕过核心保存门禁。
- 手动保存、另存为、强制保存、保存全部和自动保存共用同一个门禁。`blocksSave=true` 的当前文档核心诊断始终阻止正式持久化；项目开启“保存前校验”后，普通核心 error 也会阻止保存，但 `flow.possible-cycle` 和带 `target` 的运行目标诊断仍不阻止保存。
- 保存被阻止时不会调用 `SaveGraph/ForceSaveGraph`，不会改动原文件或 `.bak`，标签继续保持 dirty；桌面端会在应用配置目录的 `recovery/` 下原子写入恢复快照。同一源文件或未命名标签最多保留最新 5 份，超过 30 天自动清理。
- 启动时发现恢复快照会提供“恢复、保留、删除”。恢复内容进入受保护的 dirty 标签，不会直接运行或覆盖原路径；修复致命问题后应另存为新文件。正式保存成功后清理对应恢复快照。
...
- 桌面校验还会走与运行时相同的 Go 编译器规则。校验沙箱只加载当前图实际引用的 workspace 函数及其传递依赖，无关目录中的函数错误不会污染当前图；被直接或间接引用的函数仍按真实引擎规则编译。原生文档中带 `legacyClass + legacyInputs/legacyOutputs` 的兼容节点，在外部节点 schema 缺失时会用文档内嵌端口生成仅供本次校验的 fallback definition；真实 `nodes/*.json` 始终优先。该 fallback 只能证明文档结构可以编译，不能代替生产宿主注册同名 Go 工厂。Go 诊断标记为 `target.go`，可以设置 `blocksRun`，但不能设置 `blocksSave`；未来其他语言目标遵循相同边界。浏览器模式没有 Go engine，只能返回结构解析结果并报告 `engine.unavailable` 警告，不能作为上线前编译验证。

### 4.5 蓝图合法性与逻辑检查

核心分析器只依赖 `GraphDocument`、端口 schema 和语言无关控制流语义，不依赖 Go 节点工厂。可达性、数据活性和强连通分量分析均使用迭代遍历，深图不会因递归调用栈溢出；一次检查会返回全部已确认问题，而不是只返回第一个错误。

- `flow.unreachable-node`：带执行语义的节点无法从任何入口到达。默认允许保存，开启“保存前校验”后阻止保存。
- `flow.unused-data-node`：纯数据节点或数据岛没有被任何可达执行路径消费。它是警告，不阻止保存。
- `connection.multiple-producers`：同一数据输入有多个生产者；`flow.exec-fanout`：同一普通 Exec 输出直接连接多个目标。两者都属于确定的核心结构错误并阻止保存和运行。
- `flow.data-cycle`：数据依赖图存在自环或多节点强连通分量；`flow.exec-cycle`：规范化结构化循环后仍存在 Exec 自环或强连通分量。每条诊断携带完整、稳定排序的 `nodeIds`，并阻止保存和运行。
- 内建 ForLoop、While 和已证明来源位于对应循环体内部的 ForLoopBreak 不会被误报为非结构化死循环。来自循环体外部或绕过循环体的 break 回边仍按 `flow.exec-cycle` 处理。
- 未知或 opaque legacy 节点缺少可靠控制流语义时只报告 `flow.possible-cycle`。该诊断即使在严格保存模式下也不阻止保存，避免未来语言扩展或旧文件内容被误杀。
- 缺少入口、跨入口数据连接等普通逻辑问题继续显示在问题面板中；是否在严格模式下阻止保存由结构化诊断字段决定，不解析本地化错误文案。
...
当前 `Array` 保持兼容优先的扁平动态数组语义：旧蓝图可以继续混合写入数字、布尔值和字符串，混合类型本身不阻止保存。运行时在 `Port` 内部记录新解析元素的实际标量类型，不改变 `ArrayData`、`.obp` 或 `.vgf` 格式；`GetArrayInt`、`GetArrayString`、`ForeachIntArray` 等类型明确的节点读取到不兼容元素时返回执行错误，不做截断或用零值冒充成功。数字字符串继续兼容 Integer 与 String 两种读取，宿主直接构造且无法判定零值类型的旧 `PortArray` 继续使用原有字段语义。对象、嵌套数组、`null` 等当前不支持的数组元素会产生明确的校验/编译错误，不再被静默丢弃；该错误阻止执行，但不阻止保存，以便修复和兼容导入内容。

整数端口和整数变量默认值支持完整的 Go `int64` 范围。为兼容旧文件和 JavaScript：`Number` 可精确表示的整数继续保存为 JSON number，新输入或编辑的超安全范围整数在 `.obp/.obpf` 中保存为十进制字符串；读取旧文件中未加引号的超安全范围整数时，编辑器使用内部精确整数标记保留原始十进制文本及 JSON number 类型。已知 Integer 控件会把该标记转换为可编辑的十进制字符串，拒绝输入小数、指数形式和超出 int64 范围的内容；旧文件中已有的非法值会保留并用红框标识，只有用户提交合法整数后才更新，避免打开文件即静默改写。未知 legacy 字段和 `Any` 数据在打开、撤销及保存后仍输出原数值 token，不会被转成字符串或经过 JavaScript `Number` 丢失精度。Go 校验器和编译器同时接受 number 与十进制字符串两种 Integer 表示，导出 `.vgf` 时恢复为旧格式的 JSON 数字。旧编辑器保存的节点 Integer 空字符串继续按 `0` 读取并在编辑时归一化；Integer 变量的空字符串仍是严重错误。小数和超出 int64 范围的值也属于严重默认值错误。`itemType` 元数据和嵌套数组暂不写入现有文件格式，待类型契约稳定后再以可选字段设计，缺省值必须继续表示 legacy 任意项。
