...
当前 `Array` 保持兼容优先的扁平动态数组语义：旧蓝图可以继续混合写入数字、布尔值和字符串，混合类型本身不阻止保存。运行时在 `Port` 内部记录新解析元素的实际标量类型，不改变 `ArrayData`、`.obp` 或 `.vgf` 格式；`GetArrayInt`、`GetArrayString`、`ForeachIntArray` 等类型明确的节点读取到不兼容元素时返回执行错误，不做截断或用零值冒充成功。数字字符串继续兼容 Integer 与 String 两种读取，宿主直接构造且无法判定零值类型的旧 `PortArray` 继续使用原有字段语义。整数端口和整数变量默认值只接受有限、无小数且不溢出 64 位范围的值，避免把 `2.5` 静默截断为 `2`。`itemType` 元数据和嵌套数组暂不写入现有文件格式，待类型契约稳定后再以可选字段设计，缺省值必须继续表示 legacy 任意项。
