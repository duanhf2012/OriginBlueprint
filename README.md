...
整数端口和整数变量默认值支持完整的 Go `int64` 范围。为兼容旧文件和 JavaScript：`Number` 可精确表示的整数继续保存为 JSON number，超出该安全范围的整数在 `.obp/.obpf` 中保存为十进制字符串；读取旧文件中未加引号的 int64 数字时也会在 `JSON.parse` 前保留其精确文本。Go 校验器和编译器同时接受这两种表示，导出 `.vgf` 时再恢复为旧格式的 JSON 数字。小数和超出 int64 范围的值仍属于严重默认值错误。`itemType` 元数据和嵌套数组暂不写入现有文件格式，待类型契约稳定后再以可选字段设计，缺省值必须继续表示 legacy 任意项。
