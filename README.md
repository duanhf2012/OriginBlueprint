...
- 变量分组只用于编辑器内组织变量，不参与运行时语义；局部与全局变量共用分组定义。没有自定义分组时编辑器直接显示变量列表，避免额外的 Default 层级；删除自定义分组会明确列出受影响的局部和全局变量，并把它们移动到 Default。
- 旧 `.vgf` 的变量 `group` 会迁移为当前分组，并在导回 `.vgf` 时按当前变量与分组状态写回；变量中的未知 legacy 扩展字段继续保留。分组名称按忽略大小写判重，避免编辑器归一化时静默合并内容。
