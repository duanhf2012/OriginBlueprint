...
当前 `Array` 保持兼容优先的扁平动态数组语义：旧蓝图可以继续混合写入数字、布尔值和字符串，数组项不会在每次读取时做隐式全类型转换；数字字符串的既有兼容读取规则仍由具体端口决定。整数端口和整数变量默认值只接受有限、无小数且不溢出 64 位范围的值，避免把 `2.5` 静默截断为 `2`。`itemType` 元数据和嵌套数组暂不写入现有文件格式，待类型契约稳定后再以可选字段设计，缺省值必须继续表示 legacy 任意项。
