...
- 桌面校验还会走与运行时相同的 Go 编译器规则。Go 诊断标记为 `target.go`，可以设置 `blocksRun`，但不能设置 `blocksSave`；未来其他语言目标遵循相同边界。浏览器模式没有 Go engine，只能返回结构解析结果并报告 `engine.unavailable` 警告，不能作为上线前编译验证。
