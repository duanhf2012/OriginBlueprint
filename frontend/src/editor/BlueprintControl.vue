...
const props = defineProps<{ data: { value?: unknown; itemType?: 'string' | 'number'; mode?: 'open' | 'save'; setValue: (value: unknown) => void } }>()
...
const isFile = computed(() => props.data.mode === 'open' || props.data.mode === 'save')
...
async function chooseFile() {
  const path = await platform.chooseDataFile(props.data.mode ?? 'open')
  if (path) value.value = path
}
</script>

<template>
  <div v-if="isFile" class="file-control" @pointerdown.stop @dblclick.stop><input v-model="value" :title="String(value)" /><button :title="data.mode === 'save' ? 'Select output file' : 'Select input file'" @click="chooseFile">…</button></div>
  <label v-else-if="isBoolean" class="boolean-control" @pointerdown.stop @dblclick.stop><input v-model="value" type="checkbox" /><span>{{ value ? 'True' : 'False' }}</span></label>
...
.file-control { width: 145px; display: flex; }.file-control input { min-width: 0; flex: 1; height: 20px; padding: 1px 4px; border: 1px solid #777; background: #f3f3f3; color: #171717; font: 11px Consolas, monospace; }.file-control button { width: 25px; height: 20px; padding: 0; border: 1px solid #666; background: #333; color: #ddd; }
