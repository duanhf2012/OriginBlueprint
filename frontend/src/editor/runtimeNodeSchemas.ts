...
const legacyNodeSpecs: Record<string, LegacyNodeSpec> = {
  ...
  EqualString: { typeId: 'origin.flow.equal-string', inputs: ['exec', 'a', 'b'], outputs: ['false', 'true'] },
  ...
  Delay: { typeId: 'origin.flow.delay', inputs: ['exec', 'duration'], outputs: ['completed'] },
	CreateTimer: { typeId: 'origin.timer.create', inputs: ['exec', 'duration', 'looping', '', 'timerKey'], outputs: ['created', 'triggered'] },
