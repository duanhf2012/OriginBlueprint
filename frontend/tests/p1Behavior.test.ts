...
import { normalizeNodeInputDefault, type RestoreLossReport } from '../src/editor/document'
import {
...
describe('node input default normalization', () => {
  it('uses zero for blank numeric controls while preserving an empty string default', () => {
    expect(normalizeNodeInputDefault('integer', '')).toBe(0)
    expect(normalizeNodeInputDefault('float', '')).toBe(0)
    expect(normalizeNodeInputDefault('string', '')).toBe('')
    expect(normalizeNodeInputDefault('integer', 12)).toBe(12)
  })
})

describe('input port visual state', () => {
  it('keeps unconnected zero-like defaults hollow while preserving filled non-default values', () => {
    const target = {
      id: 'target',
      inputs: {
        zero: { control: { value: 0 } },
        empty: { control: { value: '' } },
        disabled: { control: { value: false } },
        count: { control: { value: 10 } },
    ...
    refreshNodePortStates([target], [], id => id === target.id ? target : undefined)

    expect(target.portStates?.inputs.zero.filled).toBe(false)
    expect(target.portStates?.inputs.empty.filled).toBe(false)
    expect(target.portStates?.inputs.disabled.filled).toBe(false)
