...
nodes/*.json
  ...
  -> Blueprint.Create 创建带独立变量的实例
  -> Blueprint.Start/Do 创建单次 Execution
  -> VM 执行或 Yield 后等待恢复
```
...
| `GraphInstance` | `Create` 产生的实例及实例变量 | 由 `Blueprint` 管理 |
| `Execution` | 一次入口调用的状态、结果和取消生命周期 | 每次调用独立 |
...
- 普通图变量属于 `Create` 出来的实例；同一实例的多次 Execution 可以看到该实例变量状态。
- 不同 `Create` 实例变量互相隔离。
...
- `HotReload` 编译成功后替换图；失败时保留旧图。
- 热加载迁移同名且类型一致的实例变量；新变量用新默认值，删除或改类型的变量不保留旧值。
- 已经运行中的 Execution 继续使用启动时捕获的 runtime state。
