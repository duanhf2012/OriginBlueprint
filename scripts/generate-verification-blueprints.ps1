...
# 01: legacy 格式。主路径是嵌套序列、For 和整数数组循环；其余节点组成可视化陈列区。
$legacyNodes = @(
...
$controlNodes = @(
    ...
    (Node 'sequence' 'origin.flow.sequence' 300 130 @{} @{ label = '四路序列'; dynamicOutputCount = 4 }),
...
WriteArtifact '02_control_flow_maze.obp' (NativeDocument '控制流迷宫' $controlNodes $controlLinks @(
    ...
    (New-GraphGroup 'flow-gallery' '控制流节点陈列：比较、Switch 与任意数组循环' 35 1070 1360 360 @('greater','less','equal','switch_legacy','switch_new','string_items','foreach_any','cast_any','any_text'))
))
...
$arrayNodes = @(
    ...
    (Node 'sequence' 'origin.flow.sequence' 290 120 @{} @{ label = '数组实验序列'; dynamicOutputCount = 3 }),
...
$arrayLinks = @(
    ...
    (Link 'append_integer' 'array' 'get_integer' 'array'),
...
$algorithmNodes = @(
    ...
    (Node 'sequence' 'origin.flow.sequence' 290 120 @{} @{ label = '评分算法序列'; dynamicOutputCount = 3 }),
...
WriteArtifact 'functions/10_score_kernel.obpf' (NativeDocument '评分核心' $scoreNodes $scoreLinks @(
    (New-GraphGroup 'score-main' '函数主路径：输入、计算和返回' 35 55 1740 240 @('entry','add','multiply','range','switch','probability','tier','return')),
...
$arrayFunctionNodes = @(
    ...
    (Node 'local_set' 'origin.variable.set' 580 520 @{ value = 0 } @{ variableId = 'fold_sum'; variableAccess = 'set'; label = 'Set FoldSum' }),
...
$arrayFunctionLinks = @(
    ...
    (Link 'summary' 'value' 'return' 'output_summary')
)
...
WriteArtifact 'functions/11_array_fold_and_format.obpf' (NativeDocument '数组折叠与格式化' $arrayFunctionNodes $arrayFunctionLinks @(
    (New-GraphGroup 'fold-main' '函数主路径与数组循环' 35 50 1740 240 @('entry','loop','sum','count_loop','get_item','append','cast','summary','return')),
    (New-GraphGroup 'fold-nested' '嵌套函数调用和局部变量' 500 470 850 180 @('score_call','local_set'))
...
$localFunctionNodes = @(
    ...
    (Node 'add' 'origin.math.add-integer' 580 120 @{ b = 1 }),
...
$localFunctionLinks = @(
    (Link 'entry' 'exec' 'set_after' 'exec'),
...
WriteArtifact 'functions/13_local_state_isolation.obpf' (NativeDocument '局部状态隔离' $localFunctionNodes $localFunctionLinks @(
    (New-GraphGroup 'local-main' '局部变量读写与函数返回' 35 50 1500 240 @('entry','get_before','add','set_after','return')),
...
$orchestratorNodes = @(
    ...
    (Node 'local_call_b' 'origin.function.call' 820 830 @{ input_seed = 7 } (FunctionCallProperties 'functions/13_local_state_isolation.obpf' '局部状态隔离' $localSignature)),
    (Node 'local_return_a' 'origin.result.append-integer' 1100 770),
    (Node 'local_return_b' 'origin.result.append-integer' 1100 900)
)
$orchestratorLinks = @(
    (Link 'entry' 'exec' 'sequence' 'exec'),
    ...
    (Link 'local_call_a' 'exec' 'local_call_b' 'exec'),
...
WriteArtifact '05_function_orchestrator.obp' (NativeDocument '函数编排主图' $orchestratorNodes $orchestratorLinks @(
    ...
    (New-GraphGroup 'local-isolation' '同输入连续调用：局部状态必须隔离' 480 760 980 220 @('local_call_a','local_call_b','local_return_a','local_return_b'))
))
...
$timerNodes = @(
    ...
    (Node 'timer_call' 'origin.function.call' 580 520 @{ input_seed = 11 } (FunctionCallProperties 'functions/13_local_state_isolation.obpf' '局部状态隔离' $localSignature)),
    (Node 'timer_result' 'origin.result.append-integer' 870 520),
    (Node 'timer_text' 'origin.literal.string' 580 700 @{ value = 'timer-event-path' }),
    (Node 'timer_text_return' 'origin.result.append-string' 870 700)
)
$timerLinks = @(
    (Link 'entry' 'exec' 'sequence' 'exec'),
    ...
    (Link 'timer_debug' 'exec' 'timer_call' 'exec'),
...
WriteArtifact '06_timer_lifecycle.obp' (NativeDocument '定时器生命周期' $timerNodes $timerLinks @(
    ...
    (New-GraphGroup 'timer-event' 'Timer 事件入口：回调处理和函数调用' 35 470 1050 320 @('timer_entry','timer_debug','timer_call','timer_result','timer_text','timer_text_return'))
))
