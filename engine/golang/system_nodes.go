...
const (
	// EntranceIDIntParam is the legacy two-integer entrance id.
	EntranceIDIntParam int64 = 1
	// EntranceIDArrayParam is the legacy array-parameter entrance id.
	EntranceIDArrayParam int64 = 2
	// EntranceIDTimer is triggered by CreateTimer callbacks.
	EntranceIDTimer int64 = 3
	returnVariable        = "g_Return"
)

// BuiltinExecNodeFactories returns all runtime nodes implemented from nodes/*.json.
...
// NewExecNodeFactory adapts a concrete node type to the factory shape used by loaders.
...
// EntranceIntParam starts legacy graphs with integer arguments.
type EntranceIntParam struct{ BaseExecNode }

// EntranceArrayParam starts legacy graphs with object id plus array arguments.
type EntranceArrayParam struct{ BaseExecNode }

// EntranceTimer starts timer callback graphs.
...
// DebugOutput is a no-op server-safe debug node.
...
// Sequence runs every connected exec output in order.
...
// CreateTimer delegates server timers to IBlueprintModule when available.
type CreateTimer struct{ BaseExecNode }

// CloseTimer cancels timers created by CreateTimer.
