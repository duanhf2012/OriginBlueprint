...
// FunctionEntranceID is the fixed entrance id used when calling a function graph.
const FunctionEntranceID int64 = 1

// MaxFunctionCallDepth prevents accidental recursive blueprint calls from
// exhausting the goroutine stack on a server.
const MaxFunctionCallDepth = 64

// FunctionEntry exposes function arguments as output data ports.
type FunctionEntry struct {
	BaseExecNode
}

// FunctionReturn collects function outputs and resumes the suspended caller.
type FunctionReturn struct {
	BaseExecNode
}

// FunctionCall invokes a compiled function graph and waits through a continuation.
...
func (n *FunctionCall) Exec() (int, error) {
	...
	// Function graphs run in their own execution session, but they intentionally
