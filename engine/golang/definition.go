...
// ExecDefinitionConfig is one node definition entry from nodes/*.json.
...
// PortDefinition describes one input or output port from definition JSON.
...
// LoadDefinitionsJSON registers node definitions from editor JSON files.
//
// Some current node files contain malformed localized strings, so the loader
// falls back to a lenient parser that extracts only names and ports.
