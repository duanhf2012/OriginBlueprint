...
func buildVerificationMatrixReport(results []verificationReportResult) []byte {
	...
	report.WriteString("补充生命周期回归测试覆盖两种变量作用域：旧文件未声明 scope 时仍为每次执行重置的局部变量；显式实例变量在同一 `graphID` 的顺序与重叠执行间共享、不同实例隔离，并覆盖热更新稳定 ID/类型迁移和 race 检查。这些生命周期用例不计入上方 13 个随机算法蓝图统计。\n\n")
	...
	report.WriteString("## 配套非随机回归\n\n")
	report.WriteString("以下用例由常规 `go test ./...` 和前端测试执行，不计入上方随机对比统计：\n\n")
	report.WriteString("- 工作区 `<workspace>/nodes` 加载、来源覆盖、目录去重和非法工作区诊断；\n")
	report.WriteString("- F5 `target.go` 使用工作区自定义节点 schema 编译；\n")
	report.WriteString("- 打开或切换工作区时刷新节点库，并丢弃过期异步加载结果；\n")
	report.WriteString("- 文件浏览器周期刷新不触发节点库重载。\n\n")
